...
public class Lector extends JDialog {
		...
		EventQueue.invokeLater(new Runnable() {
			...
			public void run() {
				try {
					...
					prodPrueba.add(new Producto(1, "Producto almacén", "aaa",
							estado));
					prodPrueba.add(new Producto(2, "Producto almacén", "aaa",
							estado));
					prodPrueba.add(new Producto(3, "Producto almacén", "aaa",
							estado));
	...
	public Lector(Bulto bultoARecoger) {
		...
		modoEjecución = SeleccionarModoEjecucion();
	...
	private JTextArea getTxA_Informacion() {
		if (txA_Informacion == null) {
			...
			txA_Informacion
					.setText("Introduzca el c\u00F3digo de barras a continuaci\u00F3n para verificar la operación");
	...
	private void crearNuevoLector() {
		if (quedaAlgunoPorRecoger() && modoEjecución == RECOGER) {
			ArrayList<Producto> productosNuevos = new ArrayList<Producto>();
			for (Producto p : productos.getProductos()) {
				if (p.getEstado_producto() == Producto.POR_RECOGER) {
					productosNuevos.add(p);
			...
			if (quedaAlgunoPorEmpaquetar() && modoEjecución == EMPAQUETAR) {
				ArrayList<Producto> productosNuevos = new ArrayList<Producto>();
				for (Producto p : productos.getProductos()) {
					if (p.getEstado_producto() == Producto.RECOGIDO) {
						productosNuevos.add(p);
				...
				JOptionPane.showMessageDialog(null,
						"No quedan más productos que "
								+ ((modoEjecución == RECOGER) ? "recoger"
										: "empaquetar")
								+ ". Saliendo del programa");
			if(modoEjecución == EMPAQUETAR)
